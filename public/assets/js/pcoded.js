...
function layout_change(layout) {
  ...
  if (layout == 'dark') {
    ...
    if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
      document.querySelector('.pc-sidebar .m-header .logo-lg').setAttribute('src', '../assets/images/logo-RSPO.png');
    }

    if (document.querySelector('.navbar-brand .logo-lg')) {
      document.querySelector('.navbar-brand .logo-lg').setAttribute('src', '../assets/images/logo-RSPO.png');
    }
    if (document.querySelector('.auth-main.v1 .auth-sidefooter')) {
      document.querySelector('.auth-main.v1 .auth-sidefooter img').setAttribute('src', '../assets/images/logo-RSPO.png');
    }
    if (document.querySelector('.footer-top .footer-logo')) {
      document.querySelector('.footer-top .footer-logo').setAttribute('src', '../assets/images/logo-RSPO.png');
    }
...
// download excel dengan filter tanggal dan konfirmasi
document.getElementById('downloadExcel').addEventListener('click', function () {
  ...
  if (startDate && endDate) {
    message = `Anda akan mendownload data dari <b>${startDate}</b> sampai <b>${endDate}</b>. Lanjutkan?`;
  } else {
    message = 'Tidak ada filter tanggal. Semua data akan didownload. Lanjutkan?';
  }
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      // Misalnya: redirect ke endpoint yang meng-generate file Excel
      let url = '#';
      if (startDate && endDate) {
...
document.querySelectorAll('.btn-danger').forEach(function (button) {
  button.addEventListener('click', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Lakukan aksi hapus (misalnya panggil API atau redirect)
        Swal.fire(
          'Terhapus!',
        ...
        // Contoh: jika mau redirect ke URL hapus
        // window.location.href = `hapus.php?id=${idData}`;
      }
    });
  });
});
